...
> **⚠️ 残余风险更深（critic 提醒，必读）**：UNSIGNED↔SIGNED 二选一**只是第一层**——驱动枚举仅 signed-**INTEGER**，而我方是 signed-**fractional（Q15×Q31）**，二者非平凡映射（需 ×2 缩放 + decimate 后处理，`fira_fit_assessment.md:130` / DOC-S4-FIRA-IMPL-01 §3 R14-3）。选对 SIGNED 仅必要非充分，**bit 对齐必上板逐位验**（crc==0x90556BC7）。
