...
| 块 | 目标 | 升级 |
|---|---|---|
| **A 聚焦增量** | 同 build A/B：8ch FIRA 链 (analyze + per-sb focus FIR + synth) 一帧 focus-ON vs OFF | 86–144 MCPS [L4] → [L1] |
...
### 2.1 聚焦 FIR 选择 + 映射 2.88 MMAC/s（justify）
- 实现 = **8-tap 分数延迟 FIR，per-subband（4 子带）per-channel**，作用在 analyze 输出的 4 子带（analyze 与
  synthesize 之间），在抽取后的子带率上。
- **MMAC 映射**：8 tap × 8 ch × Σ(子带率 3+6+12+24=45kHz) = **2.88 MMAC/s = EXACTLY** dsp_8ch_report.md:59
  「分数延迟 FIR × 4 子带, 8 ch = 2.88 MMAC/s」。 → harness 测的是**被定价的 envelope**，非夸大值。
...
| **A. `g_h1_cyc_focus_only`** | = focus − nofocus；**focus MCPS = 值×750/1e6 应落 86–144 [L4] 包络验证区** | 远超 144→实现比 2.88 重（查 tap/子带映射）；≈0→focus 没真算（查 FG） |
...
**off-board 算（C9，连体 §8 呈现）**：focus MCPS = g_h1_cyc_focus_only×750/1e6（验 86–144 [L4]→[L1]）；
