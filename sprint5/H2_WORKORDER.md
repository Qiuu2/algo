...
**【WO-S5-H2R 重测读数 worksheet 增补（R30）】**：① 调用序——`h2r_isr_clean_retest()` 须在
`g_h2_done` 之后（bench_main 接线时明写，h2r 复用 s_h2_fa 自带重 init，序保护双保险）；② per-ISR
净成本推导（off-board，F30-MINOR-3）：per-ISR(warm) = (cyc_frame_isr_smax − base)/帧内 ISR 数
（=count_in_span 折算），与「产品最小 handler（count+ack）」预期量级对照——**率在带但 per-ISR 异常重
仍须解释后才入账**（堵「率对 handler 重」残余盲区）；③ FG-B' on-board flag 假设 CCLK=1GHz class，
真 CCLK 异于此值时 flag 仅参考，须 off-board 真 CCLK 复判（RAW span+count 已出齐）。
